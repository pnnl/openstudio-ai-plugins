...
11. If installation changed runtime command availability or repaired a contract mismatch, tell the user to restart Codex or reconnect the failed MCP server. Codex discovers MCP tools only when it starts the server, so a new tool cannot appear in the current session.
