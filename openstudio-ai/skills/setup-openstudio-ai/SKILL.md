...
11. If installation changed runtime command availability or repaired a contract mismatch, tell the user to run `/reload-plugins` or reconnect the failed MCP server. Claude Code discovers MCP tools only when it starts the server, so a new tool cannot appear in the current session.
